...
"""Tests for the Triton fp8_g32 store kernel.

Mirrors `tests/kernels/fp4_g32/test_store_vs_reference.py` but with the
fp8_g32 slot layout (E8M0 scale bytes, 1 byte/group) and the UE8M0-snap
encode reference.

The Triton kernel must produce bit-identical codes and scale bytes vs
the PyTorch reference (`reference.fp8_g32_encode`).
"""
...
def test_store_matches_reference(dtype, head_dim, num_kv_heads, num_tokens):
    ...
    fp8_g32_store(key, value, kv_cache, slot_mapping)
    torch.cuda.synchronize()

    ref_k = fp8_g32_encode(key, rotate=True)
    ref_v = fp8_g32_encode(value, rotate=False)
...
def test_scale_is_power_of_two():
    ...
    from vllm.v1.attention.ops.fp8_g32.fp8_levels import UE8M0_BIAS
