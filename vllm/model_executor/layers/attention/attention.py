...
class Attention(nn.Module, AttentionLayerBase):
    ...
    def get_kv_cache_spec(self, vllm_config: VllmConfig) -> KVCacheSpec:
        ...
        # Compute TQ slot size up-front so SWA-on-TQ layers (e.g. gpt-oss)
        # can build a TQ-aware SlidingWindowSpec instead of falling back to
        # the bf16 head-size formula (which over-sizes the page and crashes
        # _reshape_kv_cache_tensors during engine init).
        tq_slot_size = 0
        ...
        #   - VLLM_TQ_DISABLE_SWA=1 (cache rotation off, kernel mask kept
        #     — produces math-equivalent output to natural mode, useful for
        #     memory-layout A/B testing)
        #   - VLLM_TQ_FORCE_FULL_ATTN=1 (cache rotation off + kernel mask
        #     also off; required so kernel never reads freed/null pages.
        #     Architecturally out-of-distribution on SWA-trained models).
        _force_full = (
            tq_slot_size > 0
            and (
                os.environ.get("VLLM_TQ_DISABLE_SWA", "0") == "1"
                or os.environ.get("VLLM_TQ_FORCE_FULL_ATTN", "0") == "1"
            )
        )

        if self.sliding_window is not None and not _force_full:
