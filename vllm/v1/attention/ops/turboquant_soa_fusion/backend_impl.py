...
class FusionTurboQuantAttentionImpl(LegacyTurboQuantAttentionImpl):
    ...
    def _ensure_on_device(self, layer, device):
        ...
        # Post-merge (f9d1307ee), `layer._tq_centroids` is no longer eagerly
        # set by Attention.__init__ — it's created lazily by the parent's
        # _ensure_on_device. Delegate to it so the base TQ attrs (centroids,
        # PiT, Pi, midpoints, _tq_cached) all get populated before our
        # SoA-fusion-specific setup runs.
        super()._ensure_on_device(layer, device)
    ...
    def _store_kv(
        ...
        # NOTE: The upstream ISOquant branch threads `centroids` as a separate
        # positional argument (and passes `layer` as a kwarg). Our base class
        # `do_kv_cache_update` calls `_store_kv(k, v, kv_cache, slot, layer)`
        # with `layer` as the 5th positional arg — same shape as our base
        # `_store_kv`. We adopt that convention and extract `centroids` from
        # `layer._tq_centroids` here so the fusion override is signature-
        # compatible with the unmodified base class.
