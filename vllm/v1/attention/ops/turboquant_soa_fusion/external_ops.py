...
def _maybe_hip_v3_mfma_like_decode(
    ...
    **kwargs,
    ...
    kv_group_size = Hq // Hk
    # Original gate — only compiled for kv_group_size=8:
    # if kv_group_size != 8:
    #     return None
    #
    # Extended: soa_bf16q_pv_mfma_decode also supports kv_group_size=6
    # (e.g. MiniMax-M2.5) via a separately compiled gqa=6 kernel.
    # _maybe_soa_bf16q_pv_mfma_decode overrides fn for gqa=6 before calling
    # here, so the gqa=8 check is relaxed to also pass for gqa=6.
    # All other callers (flash_tq, mfma_qk) still only support gqa=8.
...
def _maybe_soa_bf16q_pv_mfma_decode(*args, **kwargs):
    ...
    # )
    #
    # Extended: dispatch to the matching compiled kernel by kv_group_size.
    # KV_GROUP_SIZE is baked into the kernel at compile time (shared-memory
    # array sizes and thread counts all depend on it), so each gqa value
    # needs its own .so.  Unsupported values fall back to Triton v3.
