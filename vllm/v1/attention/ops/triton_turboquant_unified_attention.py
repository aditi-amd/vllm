...
def kernel_tq_unified_attention_2d(
    ...
    query_ptr,  # [num_tokens, Hq, D] or [num_tokens, Hq, D] rotated for MSE
...
def kernel_tq_unified_attention_3d(
    ...
    query_ptr,  # [num_tokens, Hq, D] - rotated for MSE path
...
def triton_turboquant_unified_attention(
    ...
    # Pre-rotate Q for MSE-key path. We do the rotation in fp32 for
    # numerical parity with v1/v2 (fp16 rocBLAS gemm is non-deterministic
    # due to split-K, which breaks the determinism test on small D) and
    # cast back to query's dtype so the tl.dot is fp16/bf16.
    if key_fp8:
