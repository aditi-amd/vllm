...
#
# GQA-6 sibling (Qwen3.6-27B full-attention layers, QG=6). Dispatched only when
# D == 256 and query_group_size == 6. At 256 the Q load covers exactly QG*32=192
# = 3*WARP_SIZE row-chunks (6 rows, no OOB padding), so STEP B is unchanged from
# the canonical hd256 kernel apart from the ceil() QG_LOAD_ITERS. Output stores
# are gated by ``mfma_row < QG`` so lanes 6..15 (MFMA waste) never write.
# This module implements the CANONICAL base path only:
...
Q_LDS_BYTES = 16 * HEAD_SIZE * 2                          # 8192 (full 16-row footprint)
...
# --- LDS bank-conflict padding (GQA-6 default qk_scaled + HW-TR path) --------
# The KV tile is stored [token][head_dim]. Unpadded the FP4 K row (qk_scaled A
# operand) is HEAD_SIZE//8 = 32 i32 = 128 B = exactly 32 LDS banks, and the
# bf16 V row (HW V-transpose) is HEAD_SIZE = 256 bf16 = 512 B = 4x32 banks, so
# every token in the 16-token tile starts on the same banks -> up to 16-way
# conflicts on the multi-token QK / PV LDS reads. Padding each row so its
# stride is no longer a multiple of 32 banks (128 B) breaks the conflict. The
# 16 B/row default matches the tq44v4 bf16 kernel; env-tunable for A/B.
KV_ROW_PAD_I32 = int(os.environ.get("TQ_FP8_KROW_PAD_I32", "4"))   # FP4 K pad
KV_ROW_PAD_ELEMS = int(os.environ.get("TQ_FP8_VROW_PAD", "8"))     # bf16 V pad
# FP4 K (qk_scaled): i32 slots per padded token row (32 -> 36 = 144 B).
K_ROW_I32 = (HEAD_SIZE // 8) + KV_ROW_PAD_I32
# bf16 V (HW V-transpose): padded elems / bytes per token row (256 -> 264).
V_ROW_ELEMS = HEAD_SIZE + KV_ROW_PAD_ELEMS
V_ROW_BYTES = V_ROW_ELEMS * 2
# The shared KV tile must cover the larger padded footprint. bf16 V dominates:
# 16 * 528 = 8448 B (FP4 K padded = 16 * 144 = 2304 B).
KV_TILE_LDS_BYTES_PADDED = TILE_SIZE * V_ROW_BYTES
...
def build_fp8_g32_decode_gqa6_hd256_module(
    num_seqs: int,
    num_kv_heads: int,
    num_partitions: int,
    padded_slot: int,
    ...
    canonical bf16 wide-K MFMA path runs. ``q_hoist`` only affects
    ``qk_scaled``. ``use_wht_butterfly`` is still ignored at 256.
    """
    assert query_group_size == 6, (
        f"build_fp8_g32_decode_gqa6_hd256_module is GQA-6 only; got "
        f"query_group_size={query_group_size}. Use "
        f"build_fp8_g32_decode_hd256_module from fp8_g32_decode_hd256.py for "
        f"GQA-8 or GQA-16."
    )
    ...
    QK_FP8 = bool(qk_fp8)
    V_CVT = bool(v_cvt)
    # qk_scaled: native scaled FP4xE4M3 MFMA. The instruction contracts a fixed
    # K=128, so a 256-wide head is covered by MFMA_ISSUES back-to-back issues
    # accumulating into one f32x4, each consuming its own 128-dim slice (4
    # UE8M0 groups). Replaces qk_fp8's 8 issues + 32 post-MFMA scale FMAs.
    ...
    assert HEAD_SIZE % MFMA_SCALED_K == 0, (
    ...
    QG_LOAD_ITERS = (QG * Q_GROUPS_PER_ROW + WARP_SIZE - 1) // WARP_SIZE   # 3 for QG=6@256
    ...
    # discarded lanes never read uninitialized LDS, which can hold NaN
    # patterns. At QG=6 this returns 5120 B/CTA, taking 16960 -> 11840 and
    # LDS-limited residency from 9 to 13 CTAs/CU.
    ...
    # --- LDS layout ---
    allocator = SmemAllocator(None, arch=arch, global_sym_name="fp8_g32_gqa6_hd256_smem")
    centroid_off = 0
    ...
    @flyc.kernel
    def fp8_g32_decode_gqa6_hd256_kernel(
        out_ptr: fx.Tensor,
        ...
        try:
            ...
            for n_tile in range_constexpr(16):
                ...
                if const_expr(QK_FP8):
                    ...
                    # UNSCALED; UE8M0 group scale staged to scale_lds for the
                    # post-MFMA fold. Lane owns head_dims chunk*64..+63 (2 grps).
                    for hf in range_constexpr(HALVES):
                    ...
                    for hf in range_constexpr(HALVES):
                        ...
                        vector.store(
                            ...
                            [arith.index_cast(
                                T.index,
                                tok_in_tile * fx.Int32(K_ROW_I32)
                                + grp * fx.Int32(4),
                            )],
                        )
                        scale_lds_i32.store(
                ...
                if const_expr(QK_FP8):
                    # qk_fp8 QK: 8× mfma_f32_16x16x32_fp8_fp8 (K=32 == one UE8M0
                    # group) + per-token post-MFMA UE8M0 scale fold.
                    qk_acc = zero_v4
                    for chk in range_constexpr(QK_K_CHUNKS):
                        k_idx_i64 = (
                ...
                if const_expr(USE_HW_TR):
                    v_lds_elem_base = (
                        tok_in_tile * fx.Int32(V_ROW_ELEMS)
                ...
                if const_expr(USE_HW_TR):
                    ...
                    v_lane_byte = (
                        fx.Int32(kv_off)
                        + token_idx * fx.Int32(V_ROW_BYTES)
                        + hd_sub * fx.Int32(2)
                    )
                    for h in range_constexpr(PV_N_CHUNKS):
    ...
    return fp8_g32_decode_gqa6_hd256_kernel
