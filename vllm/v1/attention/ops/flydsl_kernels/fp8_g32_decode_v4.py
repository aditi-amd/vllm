...
# fp8_g32 decode v4 (FlyDSL) — MI355X / gfx950 / CDNA4
#
# Decode-only paged attention over the fp8_g32 (FP4 E2M1 + UE8M0 group scale)
# AoS KV cache. Targets Qwen-class models: HEAD_SIZE=128, GQA group in {8,16},
...
# === Constants (Qwen-class fp8_g32 decode profile) ==========================
HEAD_SIZE = 128
KV_BLOCK_SIZE = 16          # default; overridable via build_fp8_g32_decode_v4_module(kv_block_size=...)
...
QUERY_GROUP_SIZE = 16        # default for tests; overridable via build_fp8_g32_decode_v4_module(query_group_size=...)
WARP_SIZE = 64
NUM_WARPS = 1
...
def build_fp8_g32_decode_v4_module(
    num_seqs: int,
    num_kv_heads: int,
    num_partitions: int,
    padded_slot: int,
    ...
    # --- LDS layout ---
    allocator = SmemAllocator(None, arch=arch, global_sym_name="fp8_g32_v4_smem")
    centroid_off = 0
    ...
    return fp8_g32_decode_v4_kernel
