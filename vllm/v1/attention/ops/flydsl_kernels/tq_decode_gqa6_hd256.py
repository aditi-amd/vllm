...
# TurboQuant decode GQA-6 sibling (FlyDSL) — HEAD_SIZE=256 variant
#
# GQA-6 + HEAD_SIZE=256 combination (Qwen3.6-27B / Qwen3.5 hybrid full-attn
# layers: 24 query heads / 4 KV heads, head_dim=256). Structurally a direct
# copy of ``tq_decode_hd256.py`` (the GQA-{8,16} HEAD_SIZE=256 kernel) with the
# QG knob turned to 6 — exactly mirroring how ``tq_decode_gqa6.py`` relates to
# ``tq_decode.py`` at HEAD_SIZE=128. All cross-lane ops (softmax xor-shuffle,
# ds_read_tr16_b64 transpose) are mfma_row-isolated and thus GQA-agnostic; the
# only GQA-6 specifics are the QG default, the round-up Q-load iteration count,
# and the full 16-row Q_LDS footprint (rows 6..15 gated out by mfma_row < QG).
# ---------------------------------------------------------------------------
# original header:
# TurboQuant decode (FlyDSL) — MI355X / gfx950 / CDNA4 — HEAD_SIZE=256 variant
#
# Decode-only paged attention over a TurboQuant SoA KV cache.
# This is the HEAD_SIZE=256 sibling of ``tq_decode.py`` (which handles the
# canonical HEAD_SIZE=128 Qwen/MiniMax profiles). It is a byte-for-byte port
# of the 128 kernel with every 128-specific lane/byte constant generalized to
# ``HEAD_SIZE`` so the same MFMA/softmax/PV structure runs over 256-wide heads
# (e.g. Qwen3.6-class, Qwen3.5-397B GQA-16). The 128 kernel is intentionally
# left untouched; this module is dispatched only when D == 256.
#
# Targets 256-wide heads: HEAD_SIZE=256, GQA group in {8,16}, MSE_BITS=4 K,
# VQB=4 V, N_CENTROIDS=16, BLOCK_SIZE in {16,32}, partitioned mode.
#
# NOTE (v1 scope): the in-register WHT butterfly (use_wht_butterfly=True) is
# NOT yet ported to 256-wide heads — the 128 butterfly is structurally tied to
# 2 elements/lane and 6 cross-lane stages (128 = 64 lanes x 2). A 256 head is
# 4 elements/lane and needs an extra stage, which is a separate follow-up.
# The launcher-side q@PiT GEMM rotation path (use_wht_butterfly=False) is fully
# supported and is what delivers the capacity/accuracy parity win.
#
# === Design ===
...
# === Constants (256-wide-head TQ decode profile) ============================
HEAD_SIZE = 256
# default; overridable via build_tq_decode_hd256_module(kv_block_size=...)
KV_BLOCK_SIZE = 16
...
QUERY_GROUP_SIZE = 6  # GQA-6 default (Qwen3.6-27B full-attention layers)
WARP_SIZE = 64
NUM_WARPS = 1
...
# The K/V dequant lane assignment splits each token's packed bytes across the
# 4 ``chunk_in_tok`` sub-lanes (lane % 4). Each lane therefore processes
# ``KEY_DATA_BYTES // 4`` packed bytes = ``LANE_KEY_WORDS`` i32 words, each
# word holding 8 nibbles (8 dequantized head-dim elements).
#   HEAD_SIZE=128: LANE_KEY_BYTES=16, LANE_KEY_WORDS=4, SUBCHUNK_HDIMS=32
#   HEAD_SIZE=256: LANE_KEY_BYTES=32, LANE_KEY_WORDS=8, SUBCHUNK_HDIMS=64
LANE_KEY_BYTES = KEY_DATA_BYTES // 4  # packed K bytes per lane per K-tile
LANE_KEY_WORDS = LANE_KEY_BYTES // 4  # i32 words per lane
# AMD buffer_load maxes at dwordx4 (16 B). At 256 a lane's 32 B needs 2 loads.
HALVES = LANE_KEY_BYTES // 16  # 1 for HEAD_SIZE=128, 2 for HEAD_SIZE=256
HALF_I64 = 8  # i64 slots written per 16-B half (4 i32 words -> 8 bf16 -> ... )
HALF_HDIMS = 32  # head-dims dequantized per 16-B half (4 words x 8 nibbles)
SUBCHUNK_HDIMS = HEAD_SIZE // 4  # head-dims per chunk_in_tok sub-lane
I64_PER_TOKEN = HEAD_SIZE // 4  # i64 slots per token row in KV LDS
SUBCHUNK_I64 = I64_PER_TOKEN // 4  # i64 offset stride per chunk_in_tok
# Q row-major LDS load geometry (STEP B): each lane loads 8 bf16 (=4 i32),
# so a row spans ``HEAD_SIZE // 8`` column-groups. row = row_chunk >> shift.
Q_GROUPS_PER_ROW = HEAD_SIZE // 8  # 16 for 128, 32 for 256
Q_ROW_SHIFT = Q_GROUPS_PER_ROW.bit_length() - 1  # 4 for 128, 5 for 256
Q_COL_MASK = Q_GROUPS_PER_ROW - 1  # 15 for 128, 31 for 256

# LDS regions
CENTROID_LDS_BYTES = N_CENTROIDS * 4  # 64
# GQA-6 uses the full 16-row Q footprint (not QG*HEAD_SIZE) — matches the
# tq_decode_gqa6.py bugfix that avoids a multi-shape JIT NaN at the smaller
# Q region size. 16 * 256 * 2 = 8192 bytes for HEAD_SIZE=256.
...
def build_tq_decode_gqa6_hd256_module(
    num_seqs: int,
    num_kv_heads: int,
    num_partitions: int,
    ...
    assert query_group_size == 6, (
        f"build_tq_decode_gqa6_hd256_module is GQA-6 only; got "
        f"query_group_size={query_group_size}. Use build_tq_decode_hd256_module "
        f"from tq_decode_hd256.py for GQA-8 or GQA-16."
    )
    ...
    # decode must accept larger blocks than the {16,32} MiniMax/Qwen profiles.
    assert kv_block_size in (16, 32, 64, 128, 256), (
        f"kv_block_size must be one of 16/32/64/128/256; got {kv_block_size}"
    )
    assert kv_block_size % TILE_SIZE == 0
    assert (
        KV_COMPUTE_BLOCK % kv_block_size == 0
        or kv_block_size % KV_COMPUTE_BLOCK == 0
    ), (
        f"kv_block_size ({kv_block_size}) must evenly tile or be a multiple of "
        f"KV_COMPUTE_BLOCK ({KV_COMPUTE_BLOCK})"
    )
    if bool(use_wht_butterfly):
        # The in-register WHT butterfly is not yet ported to 256-wide heads
        # (see module header). Callers must use the launcher-side q@PiT GEMM
        # rotation path for HEAD_SIZE=256.
        raise NotImplementedError(
            "tq_decode_hd256: use_wht_butterfly=True is not supported for "
            "HEAD_SIZE=256 yet; use the q@PiT GEMM rotation path "
    ...
    # distributed WARP_SIZE per iteration. Round UP so GQA-6 (192 groups at
    # HEAD_SIZE=256 → exactly 3 iters) and any partial layout are covered;
    # over-read rows are OOB-protected and gated out by mfma_row < QG.
    ...
    def tq_decode_gqa6_hd256_kernel(
        ...
        if not use_wht_butterfly:
            ...
            for c in range_constexpr(QG_LOAD_ITERS):
                row_chunk = lane + fx.Int32(c * WARP_SIZE)  # 0..QG*Q_GROUPS-1
                row = row_chunk >> fx.Int32(Q_ROW_SHIFT)  # 0..QG-1
                col_b = row_chunk & fx.Int32(Q_COL_MASK)  # 0..Q_GROUPS_PER_ROW-1
                col_elem = col_b * fx.Int32(8)  # 0..(HEAD_SIZE-8) in bf16
        ...
        try:
            ...
            for n_tile in range_constexpr(16):
                ...
                # Each lane owns ``LANE_KEY_BYTES`` packed K bytes for its
                # (token, sub-chunk). buffer_load maxes at 16 B (dwordx4), so
                # for HEAD_SIZE=256 (32 B/lane) we issue ``HALVES``=2 loads.
                # For HEAD_SIZE=128 (16 B/lane) HALVES=1 → identical to the
                # canonical kernel.
                k_byte0 = data_bases_byte + chunk_in_tok * fx.Int32(LANE_KEY_BYTES)
                k_packed_list = []
                ...
                v_byte0 = (
                    data_bases_byte + c_keydata + chunk_in_tok * fx.Int32(LANE_KEY_BYTES)
                )
                v_packed_list = []
                for hf in range_constexpr(HALVES):
                    ...
                    for hf in range_constexpr(HALVES):
                        ...
                        for w in range_constexpr(4):
                            ...
                            for n in range_constexpr(8):
                                ...
                                head_dim = chunk_in_tok * fx.Int32(
                                    SUBCHUNK_HDIMS
                                ) + fx.Int32(half_hd + w * 8 + n)
    ...
    return tq_decode_gqa6_hd256_kernel
