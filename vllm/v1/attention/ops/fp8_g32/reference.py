...
"""PyTorch reference for the fp8_g32 KV cache format.

Mirrors `fp4_g32.reference` exactly, with two changes that match the
fp8_g32 algorithm spec (see `fp8_g32/fp8_levels.py`):

1. **Scale is UE8M0 (power-of-two)** rather than fp16-cast. After
   `s = c * absmax` we snap `s = 2^round(log2(s))` and store as a single
   byte (E8M0). The encode path uses this snapped `s` as the divisor,
   so the quantized codes can differ from `fp4_g32` by at most one grid
   step (the same FP4 grid is used).

2. **Q is consumed as FP8 E4M3** in the reference attention. Q is
   first Hadamard-rotated, then cast to FP8 E4M3 with scale = 1 before
   the QK matmul. This matches the kernel launcher.

Same FP4 E2M1 grid. Same `c = 0.156`. No per-token L2 normfold. No V
rotation. K rotated, no inverse on K (Q comes pre-rotated).

This module is the bit-comparison ground truth for the Triton kernels.
"""
...
@dataclass
class FP8G32Encoded:
    """Encoded fp8_g32 representation of one tensor along its last axis.
    ...
    def scales_fp32(self) -> torch.Tensor:
        ...
        is_zero = b == 0
        s = torch.where(
            is_zero, torch.zeros_like(b, dtype=torch.float32),
...
def fp8_g32_encode(
    ...
    *,
    ...
    if x.shape[-1] % GROUP_SIZE != 0:
        raise ValueError(
            f"fp8_g32_encode: last dim {x.shape[-1]} must be a multiple of "
...
def reference_fp8_g32_attention(
    ...
    *,
    ...
    k_recon_rot = fp8_g32_encode_decode(
